...
1. **Check pod status**:
   ```bash
   oc get pods -n aialchemists-official
   ```

2. **View application logs**:
   ```bash
   oc logs -n aialchemists-official deployment/hackathon-api
   ```

3. **Access debug endpoint**:
   ```bash
   ROUTE_URL=$(oc get route hackathon-api -n aialchemists-official -o jsonpath='{.spec.host}')
   curl -k https://$ROUTE_URL/debug | jq .
   ```

4. **Run diagnostic test**:
   ```bash
   oc exec -n aialchemists-official deployment/hackathon-api -- python /app/test-wif.py
   ```